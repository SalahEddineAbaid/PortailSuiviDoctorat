...
public class EmailService {
    ...
    @Value("${notification.email.from:noreply@thesis-portal.com}")
    private String fromEmail;

    @Value("${notification.email.from-name:Thesis Management Portal}")
    private String fromName;

    /**
     * Sends an HTML email with resilience patterns applied.
     * 
     * Requirement 13.1: WHEN a notification is successfully sent THEN log at INFO
     * level
    ...
    /**
     * Fallback method for sendEmailWithAttachment when circuit breaker opens or all
     * retries fail.
     * Logs the failure and returns a completed future with exception.
     * 
     * @param to             recipient email address
