 ...
 * Provides endpoints for querying notification history, statistics, and retry operations.
 * 
...
public class NotificationController {
     ...
     * Requirement 6.1: WHEN an admin requests GET /api/notifications THEN return a paginated list of all notifications
     */
    @Operation(
        summary = "Get all notifications",
        description = "Retrieve a paginated list of all notifications. Requires ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
    ...
    public ResponseEntity<Page<Notification>> getAllNotifications(
            ...
            @Parameter(description = "Field to sort by", example = "dateCreation")
     ...
     * Requirement 6.2: WHEN an admin requests GET /api/notifications/{id} THEN return the notification with the specified ID
     */
    @Operation(
        summary = "Get notification by ID",
        description = "Retrieve a specific notification by its ID. Requires ADMIN role or ownership of the notification."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
        ...
        @ApiResponse(
            responseCode = "403",
            description = "Forbidden - User does not have permission to view this notification"
        ),
        @ApiResponse(
            responseCode = "404",
    ...
    public ResponseEntity<Notification> getNotificationById(
            @Parameter(description = "Notification ID", example = "1", required = true)
     ...
     * Requirement 6.3: WHEN a user requests GET /api/notifications/user/{email} THEN return only notifications for that email
     */
    @Operation(
        summary = "Get notifications by user email",
        description = "Retrieve all notifications for a specific user. Users can only access their own notifications unless they have ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
    ...
    public ResponseEntity<Page<Notification>> getNotificationsByUser(
            @Parameter(description = "User email address", example = "doctorant@emsi.ma", required = true)
     ...
     * Requirement 6.4: WHEN an admin requests GET /api/notifications/status/{status} THEN return all notifications with that status
     */
    @Operation(
        summary = "Get notifications by status",
        description = "Retrieve all notifications with a specific status (PENDING, SENT, FAILED, RETRYING). Requires ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved notifications by status",
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = Page.class)
            )
    ...
    public ResponseEntity<Page<Notification>> getNotificationsByStatus(
            @Parameter(description = "Notification status", example = "FAILED", required = true)
            @PathVariable StatutNotification status,
            @Parameter(description = "Page number (0-indexed)", example = "0")
            @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "Number of items per page", example = "20")
            @RequestParam(defaultValue = "20") int size) {
     ...
     * Requirement 6.5: WHEN an admin requests GET /api/notifications/stats THEN return statistics including total, sent, failed, pending counts and success rate
     */
    @Operation(
        summary = "Get notification statistics",
        description = "Retrieve aggregated statistics about notification delivery including total, sent, failed, pending counts and success rate. Requires ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
            ...
            content = @Content(
                ...
                examples = @ExampleObject(
                    ...
                    value = "{\"total\": 1000, \"sent\": 950, \"failed\": 30, \"pending\": 10, \"retrying\": 10, \"successRate\": 96.94}"
                )
            )
        ),
        @ApiResponse(
     ...
     * Requirement 6.6: WHEN an admin requests POST /api/notifications/{id}/retry THEN retry sending the failed notification
     */
    @Operation(
        summary = "Retry a failed notification",
        description = "Retry sending a failed notification. The notification must have status FAILED. Requires ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
            ...
            content = @Content(
                ...
                examples = @ExampleObject(
                    ...
                    value = "{\"success\": true, \"message\": \"Notification queued for retry\", \"notificationId\": 123, \"status\": \"RETRYING\"}"
                )
            )
        ),
        @ApiResponse(
            responseCode = "400",
            description = "Bad Request - Notification cannot be retried (not in FAILED status)"
        ),
        @ApiResponse(
            responseCode = "401",
            description = "Unauthorized - Invalid or missing JWT token"
    ...
    public ResponseEntity<Map<String, Object>> retryNotification(
            @Parameter(description = "Notification ID to retry", example = "123", required = true)
            @PathVariable Long id) {
     ...
     * Requirement 6.7: WHEN an admin requests GET /api/notifications/failed THEN return all notifications with status FAILED
     */
    @Operation(
        summary = "Get all failed notifications",
        description = "Retrieve all notifications with status FAILED. Requires ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved failed notifications",
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = Page.class)
            )
     ...
     * Requirement 6.8: WHEN an admin requests GET /api/notifications/search THEN filter notifications by destinataire, type, status, and date range
     */
    @Operation(
        summary = "Search notifications with filters",
        description = "Search and filter notifications by recipient email, type, status, and date range. All filters are optional. Requires ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved filtered notifications",
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = Page.class)
            )
    ...
    public ResponseEntity<Page<Notification>> searchNotifications(
            @Parameter(description = "Filter by recipient email", example = "doctorant@emsi.ma")
            @RequestParam(required = false) String destinataire,
            @Parameter(description = "Filter by notification type", example = "INSCRIPTION_VALIDEE_ADMIN")
            @RequestParam(required = false) TypeNotification type,
            @Parameter(description = "Filter by notification status", example = "SENT")
            @RequestParam(required = false) StatutNotification status,
            @Parameter(description = "Filter by start date (ISO format)", example = "2024-01-01T00:00:00")
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime dateDebut,
            @Parameter(description = "Filter by end date (ISO format)", example = "2024-12-31T23:59:59")
        ...
        log.info("GET /api/notifications/search - destinataire: {}, type: {}, status: {}, dateDebut: {}, dateFin: {}, page: {}, size: {}",
     ...
     * Requirement 11.4: WHEN an admin requests POST /api/notifications/dlq/retry-all THEN reprocess all messages from the DLQ
     */
    @Operation(
        summary = "Retry all DLQ messages",
        description = "Reprocess all failed notifications from the Dead Letter Queue. This will attempt to resend all notifications that failed after exhausting retries. Requires ADMIN role."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
            description = "DLQ retry operation completed",
            content = @Content(
                mediaType = "application/json",
                examples = @ExampleObject(
                    name = "Success Response",
                    value = "{\"success\": true, \"message\": \"DLQ retry operation completed\", \"totalProcessed\": 50, \"successCount\": 45, \"failureCount\": 5}"
                )
            )
        ),
        @ApiResponse(
        ...
        @ApiResponse(
            responseCode = "500",
            description = "Internal Server Error - Error during DLQ retry operation"
        )
    })
